    ...
    public class TherapyRepository : ITherapyRepository
    {
        ...
        public async Task<List<Therapy>> GetAllAsync()
        {
            return await _context.Therapies
                .AsNoTracking()
                .Where(t => t.IsActive)
                .Select(t => new Therapy(t.TherapyName)
                {
                    Id = t.TherapyId
                })
                .ToListAsync();
        ...
        {
            if (therapy is null)
                throw new ArgumentNullException(nameof(therapy));

            var entity = await _context.Therapies
                .FirstOrDefaultAsync(t => t.TherapyId == therapy.Id);

            if (entity == null)
                throw new InvalidOperationException("Therapy not found");

            entity.TherapyName = therapy.Name;
